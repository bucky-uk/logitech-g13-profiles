...
function Loadout:activate()
	if self.hKey then
		if self.hKeyMod then
			PressKey(self.hKeyMod)
			PressAndReleaseKey(self.hKey)
			ReleaseKey(self.hKeyMod)
    		else
			PressAndReleaseKey(self.hKey)
		end
	end
    ...
    if event == "G_PRESSED" then
        ...
        if not l then
            -- do nothing
        	   ...
        	   local hkeyMod = l.hKeyMod .. "+" or ""
            if not _isCasting then
                _activeKeyMap = arg
                if l.hKeyDelay and l.hKeyDelay > 0 then
                    print("=== G_PRESSED: " .. hkeyMod .. l.hKey .. " pressed, swapping to " .. l.name .. " (Longpress mode)")
                    SwapLoadoutAfterDelay(l)
                else
                    print("=== G_PRESSED: " .. hkeyMod .. l.hKey .. " pressed, swapping to " .. l.name)
                    SwapLoadout(l)
                end
            end
        end
        ...
        if not l then
            ...
            if _activeKeyMap == arg then
                if not _isSticky then
                    print("=== G_RELEASED: " .. hKeyMod .. l.hKey .. " released, reverting to " .. _previousLoadout.name)
